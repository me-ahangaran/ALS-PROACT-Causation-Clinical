...
timePosMat = csvread('D:\PHD\Thesis\Implementation\ALS-Matlab\SDU\Progression\Fast\CFD\TimePosition.txt');
...
for i=1:row
    ...
    if (ent ~= 0)
        ...
        fnameDensity = strcat('D:\PHD\Thesis\Implementation\ALS-Matlab\SDU\Progression\Fast\TimePosition\TimePosDensity.txt');
        fid_density = fopen(fnameDensity,'w');
        dlmwrite(fnameDensity,F_Mat);
        fclose(fid_density);

        fnameX = strcat('D:\PHD\Thesis\Implementation\ALS-Matlab\SDU\Progression\Fast\TimePosition\TimePosX.txt');
        fid_X = fopen(fnameX,'w');
        dlmwrite(fnameX,X_Mat);
        fclose(fid_X);
        
        fnameEntropy = strcat('D:\PHD\Thesis\Implementation\ALS-Matlab\SDU\Progression\Fast\TimePosition\TimePosEntropy.txt');
        fid_Ent = fopen(fnameEntropy,'w');
        dlmwrite(fnameEntropy,entropyProb);
        fclose(fid_Ent);
        
        fnameProb = strcat('D:\PHD\Thesis\Implementation\ALS-Matlab\SDU\Progression\Fast\TimePosition\TimePosProb.txt');
        fid_Prob = fopen(fnameProb,'w');
        dlmwrite(fnameProb,P_Mat);
        fclose(fid_Prob);
        
        fnameXP = strcat('D:\PHD\Thesis\Implementation\ALS-Matlab\SDU\Progression\Fast\TimePosition\TimePosXP.txt');
        fid_XP = fopen(fnameXP,'w');
        dlmwrite(fnameXP,XP_Mat);
        fclose(fid_XP);
        
        fnameMaxProb = strcat('D:\PHD\Thesis\Implementation\ALS-Matlab\SDU\Progression\Fast\TimePosition\TimePosMaxPointsProb.txt');
