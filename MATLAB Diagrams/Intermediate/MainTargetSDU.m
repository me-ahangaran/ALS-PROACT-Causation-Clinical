...
targetMat = csvread('D:\PHD\Thesis\Implementation\ALS-Matlab\SDU\Progression\Intermediate\CFD\Target.txt');
targetMatAbs = csvread('D:\PHD\Thesis\Implementation\ALS-Matlab\SDU\Progression\Intermediate\CFD\TargetAbs.txt');
...
for i=1:numOfFeatures
    ...
    if (entFirstRow ~= 0)
        ...
        fnameDensity = strcat('D:\PHD\Thesis\Implementation\ALS-Matlab\SDU\Progression\Intermediate\Target\TargetDensity.txt');
        fid_density = fopen(fnameDensity,'w');
        dlmwrite(fnameDensity,F_Mat);
        fclose(fid_density);
        
        %absolute target
        fnameDensityAbs = strcat('D:\PHD\Thesis\Implementation\ALS-Matlab\SDU\Progression\Intermediate\Target\TargetDensityAbs.txt');
        fid_densityAbs = fopen(fnameDensityAbs,'w');
        dlmwrite(fnameDensityAbs,F_MatAbs);
        fclose(fid_densityAbs);

        fnameX = strcat('D:\PHD\Thesis\Implementation\ALS-Matlab\SDU\Progression\Intermediate\Target\TargetX.txt');
        fid_X = fopen(fnameX,'w');
        dlmwrite(fnameX,X_Mat);
        fclose(fid_X);
        
        %absolute target
        fnameXAbs = strcat('D:\PHD\Thesis\Implementation\ALS-Matlab\SDU\Progression\Intermediate\Target\TargetXAbs.txt');
        fid_XAbs = fopen(fnameXAbs,'w');
        dlmwrite(fnameXAbs,X_MatAbs);
        fclose(fid_XAbs);
        
        fnameEntropy = strcat('D:\PHD\Thesis\Implementation\ALS-Matlab\SDU\Progression\Intermediate\Target\EntropyTarget.txt');
        fid_Ent = fopen(fnameEntropy,'w');
        dlmwrite(fnameEntropy,entropyProb);
        fclose(fid_Ent);
        
        %absolute target
        fnameEntropyAbs = strcat('D:\PHD\Thesis\Implementation\ALS-Matlab\SDU\Progression\Intermediate\Target\EntropyTargetAbs.txt');
...
%         fclose(fid_Ent);
        ...
        fnameProb = strcat('D:\PHD\Thesis\Implementation\ALS-Matlab\SDU\Progression\Intermediate\Target\TargetProb.txt');
        fid_Prob = fopen(fnameProb,'w');
        dlmwrite(fnameProb,P_Mat);
        fclose(fid_Prob);
        
        %absolute target
        fnameProbAbs = strcat('D:\PHD\Thesis\Implementation\ALS-Matlab\SDU\Progression\Intermediate\Target\TargetProbAbs.txt');
        fid_ProbAbs = fopen(fnameProbAbs,'w');
        dlmwrite(fnameProbAbs,P_MatAbs);
        fclose(fid_ProbAbs);
        
        fnameXP = strcat('D:\PHD\Thesis\Implementation\ALS-Matlab\SDU\Progression\Intermediate\Target\TargetXP.txt');
        fid_XP = fopen(fnameXP,'w');
        dlmwrite(fnameXP,XP_Mat);
        fclose(fid_XP);
        
        %absolute target
        fnameXPAbs = strcat('D:\PHD\Thesis\Implementation\ALS-Matlab\SDU\Progression\Intermediate\Target\TargetXPAbs.txt');
        fid_XPAbs = fopen(fnameXPAbs,'w');
        dlmwrite(fnameXPAbs,XP_MatAbs);
        fclose(fid_XPAbs);
        
        fnameMaxProb = strcat('D:\PHD\Thesis\Implementation\ALS-Matlab\SDU\Progression\Intermediate\Target\TargetMaxPointsProb.txt');
        fid_MaxProb = fopen(fnameMaxProb,'w');
        dlmwrite(fnameMaxProb,maxProb_Mat);
        fclose(fid_MaxProb);
        
        %absolute target
        fnameMaxProbAbs = strcat('D:\PHD\Thesis\Implementation\ALS-Matlab\SDU\Progression\Intermediate\Target\TargetMaxPointsProbAbs.txt');
...
%creating first row files
        fnameDensityFirstRow = strcat('D:\PHD\Thesis\Implementation\ALS-Matlab\SDU\Progression\Intermediate\FirstRow\FirstRowDensity.txt');
        fid_densityFirstRow = fopen(fnameDensityFirstRow,'w');
        dlmwrite(fnameDensityFirstRow,F_MatFirstRow);
        fclose(fid_densityFirstRow);

        fnameXFirstRow = strcat('D:\PHD\Thesis\Implementation\ALS-Matlab\SDU\Progression\Intermediate\FirstRow\FirstRowX.txt');
        fid_XFirstRow = fopen(fnameXFirstRow,'w');
        dlmwrite(fnameXFirstRow,X_MatFirstRow);
        fclose(fid_XFirstRow);
        
        fnameEntropyFirstRow = strcat('D:\PHD\Thesis\Implementation\ALS-Matlab\SDU\Progression\Intermediate\FirstRow\FirstRowEntropy.txt');
        fid_EntFirstRow = fopen(fnameEntropyFirstRow,'w');
        dlmwrite(fnameEntropyFirstRow,entropyProbFirstRow);
        fclose(fid_EntFirstRow);
        
        fnameProbFirstRow = strcat('D:\PHD\Thesis\Implementation\ALS-Matlab\SDU\Progression\Intermediate\FirstRow\FirstRowProb.txt');
        fid_ProbFirstRow = fopen(fnameProbFirstRow,'w');
        dlmwrite(fnameProbFirstRow,P_MatFirstRow);
        fclose(fid_ProbFirstRow);
        
        fnameXPFirstRow = strcat('D:\PHD\Thesis\Implementation\ALS-Matlab\SDU\Progression\Intermediate\FirstRow\FirstRowXP.txt');
        fid_XPFirstRow = fopen(fnameXPFirstRow,'w');
        dlmwrite(fnameXPFirstRow,XP_MatFirstRow);
        fclose(fid_XPFirstRow);
        
        fnameFirstRowMaxProb = strcat('D:\PHD\Thesis\Implementation\ALS-Matlab\SDU\Progression\Intermediate\FirstRow\FirstRowDensityMaxPoints.txt');
