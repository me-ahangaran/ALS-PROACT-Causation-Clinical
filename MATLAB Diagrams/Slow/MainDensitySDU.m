...
%         maxProb_Mat(idx,3) = prob;
        ...
        fnameDensity = strcat('D:\PHD\Thesis\Implementation\ALS-Matlab\SDU\Progression\Slow\Density\density(',mat2str(prevIdx),'-',mat2str(nextIdx),').txt');
        fid_density = fopen(fnameDensity,'w');
        dlmwrite(fnameDensity,density);
        fclose(fid_density);

        fnameProb = strcat('D:\PHD\Thesis\Implementation\ALS-Matlab\SDU\Progression\Slow\Probability\prob(',mat2str(prevIdx),'-',mat2str(nextIdx),').txt');
 ...
 fnameMaxProb = strcat('D:\PHD\Thesis\Implementation\ALS-Matlab\SDU\Progression\Slow\MaxPointsProb.txt');
 fid_MaxProb = fopen(fnameMaxProb,'w');
 dlmwrite(fnameMaxProb,maxProb_Mat);
 fclose(fid_MaxProb);
 
 fnameX = strcat('D:\PHD\Thesis\Implementation\ALS-Matlab\SDU\Progression\Slow\X.txt');
 fid_X = fopen(fnameX,'w');
 dlmwrite(fnameX,X_Mat);
 fclose(fid_X);
 
 fnameY = strcat('D:\PHD\Thesis\Implementation\ALS-Matlab\SDU\Progression\Slow\Y.txt');
 fid_Y = fopen(fnameY,'w');
 dlmwrite(fnameY,Y_Mat);
 fclose(fid_Y);
 
 fnameXp = strcat('D:\PHD\Thesis\Implementation\ALS-Matlab\SDU\Progression\Slow\Xp.txt');
 fid_Xp = fopen(fnameXp,'w');
 dlmwrite(fnameXp,Xp_Mat);
 fclose(fid_Xp);
 
 fnameYp = strcat('D:\PHD\Thesis\Implementation\ALS-Matlab\SDU\Progression\Slow\Yp.txt');
 fid_Yp = fopen(fnameYp,'w');
 dlmwrite(fnameYp,Yp_Mat);
 fclose(fid_Yp);
 
 fnameEntProb = strcat('D:\PHD\Thesis\Implementation\ALS-Matlab\SDU\Progression\Slow\EntropyDensity.txt');
